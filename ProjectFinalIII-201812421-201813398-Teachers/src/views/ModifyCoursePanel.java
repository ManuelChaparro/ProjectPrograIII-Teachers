...
public class ModifyCoursePanel extends JPanel {
	...
	private ButtonObj createCourse, acceptCourse, btnAdd;
	...
	private void createCoursesComboBox(ActionListener actionListener, JPanel containerBox) {
		...
		course.setActionCommand(Event.VISIBLE_MOD_COURSE.toString());
		course.setBackground(Color.WHITE);
		course.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(ConstantsGUI.DARK_BLUE, 1),
				"CURSOS INSCRITOS", TitledBorder.LEFT, TitledBorder.TOP, ConstantsGUI.DEFAULT_FONT_BOLD,
	...
	private void createConfirmSearchBtn(ActionListener actionListener, JPanel containerBox) {
		...
		createCourse = new ButtonObj("Confirmar", actionListener, Event.BTN_MODIFY.toString(),
				ConstantsGUI.DARK_BLUE);
	...
	private void createContainerScheduler(ActionListener actionListener) {
		...
		acceptCourse = new ButtonObj("Aceptar", actionListener, Event.CONFIRM_MODIFY_COURSE_TH.toString(), ConstantsGUI.DARK_YELLOW);
	...
	public void setDataModify(String dataModify) {
		String[] modifyVector = dataModify.split("&");
		annotation.setText(modifyVector[2]);
		String formatSchedules = modifyVector[3].replace('#', ':');
		scheduleField.setText(formatSchedules.replaceAll("%", "  ->  "));
	}
